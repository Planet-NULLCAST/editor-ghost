...
import formatMarkdown from 'ember-quickstart/utils/format-markdown';
...
import {svgJar} from 'ember-quickstart/helpers/svg-jar';
...
export default Component.extend({
    ...
    saveCardAsSnippetIfPossible: computed('saveSnippet', function () {
        console.log(this.saveCardAsSnippet)
        return this.saveCardAsSnippet
    ...
    init() {
        ...
        console.log('component cards0', this.componentCards)
    },
        ...
        let componentHooks = {
            ...
            [ADD_CARD_HOOK]: ({env, options, payload}, koenigOptions) => {
                ...
                run.schedule('afterRender', () => {
                    ...
                    console.log(this.componentCards)
                });
    ...
    actions: {
        ...
        replaceWithCardSection(cardName, range, payload) {
            console.log({cardName, payload, range})
            let editor = this.editor;
    ...
    saveCardAsSnippet: action(function (card) {
        console.log('inside savecard as snippet====>>>', card)
        let section = this.getSectionFromCard(card);
